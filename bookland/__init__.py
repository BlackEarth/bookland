...
MYNAME="bookland.py"
MYVERSION="0.94"
COPYRIGHT="(C) 1999-2005 J. Milgram"
DATE = "Jan. 2005"
MAINTAINER = "bookland-bugs@cgpp.com"

#   Copyright (C) 1999-2003 Judah Milgram     
#
#   bookland.py - generate Bookland EAN symbol for ISBN and ISMN encoding
#
...
#  HISTORY:
#
#  1/2005  -  v 0.94 Add facility to print 13 digit ISBN on top of bars instead of
#                    10 digit ISBN. Don't use this until after 1 Jan 2007, per
#                    the 13-digit implementation guidelines at
#               www.isbn-international.org/en/download/implementation-guidelines-04.pdf
#
#                    Fixed EAN-13 "right" digits to scale to available space, to
#                    make consistent with the way we handled the "left" digits.
#  9/2003  -  v 0.93 added provision to license to cover program output
...
class ISBN:
    ...
    # This is an old-style 10 digit ISBN!
    def __init__(self,arg):
    ...
    # We'll use this later when we have to start printing the 13-digit ISBN's over
    # the bars (after 1 Jan 2007).
    def isbn13(self):
        # Return the 13 digit ISBN string based on the 10-digit (978) prefix ISBN.
        # Except if it's an ISMN, we make no conversion.
        # I wonder if there is going to be a 13 digit ISMN.
        if self.name=="ISMN":
            return self.s
        else:
            arg = "978-" + self.s[:-1]
...
class barCodeSymbol:
    ...
    def psTopCenterText(self,text,font,fit=0):
        # the text at the top center of the bar pattern (i.e. the ISBN)
        # Set fit=1 if you want the string fitted to the barcode width.
        rval = [ PSFORMAT % self.patternWidth + " 72 mul" ]
        if fit:
            rval.append("dup")
        rval.append("2 div")
        rval.append(PSFORMAT % self.moduleHeight + " 72 mul 3 add moveto")
        rval.append("(%s)" % text)
        if fit:
            rval.append("/%s fitstring" % font)
        rval.append("bottomcentershow")
        return rval
...
class EAN13Symbol(barCodeSymbol):
    ...
    def psLRDigitLines(self):
        ...
        return [ "% We do the left digits first and leave the font scaled",
                 ...
                 "moduleWidth 40 mul (" + self.ean13.rightDigits + ") ",
                 "/" + self.font + " fitstring topcentershow" ]
